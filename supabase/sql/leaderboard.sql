...
--
-- NOTE: starting_weight_kg/goal_weight_kg/current_weight_kg ARE included
-- below (for the per-member detail card) — a deliberate, explicit exception
-- to "never raw weight" for opted-in leaderboard members specifically, not
-- a general relaxation. Everything else on `profiles` (height, Strava
-- tokens, etc.) still never leaves this function body.
...
-- Re-running this block is required after any DROP+CREATE, since a fresh
-- function gets default privileges again (grants don't survive a drop).
